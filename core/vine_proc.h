...
typedef struct
{
	...
	int users;
	int data_off;	/**< Offset relative to name where process binary begins(strlen(name)) */
	char name[];
 ...
 */
vine_proc_s * vine_proc_init(void * mem,const char * name,vine_accel_type_e type,const void * code,size_t code_size);
