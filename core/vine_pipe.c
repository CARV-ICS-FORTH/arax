...
vine_pipe_s * vine_pipe_init(void * mem,size_t size,size_t ring_size)
{
	...
	__sync_val_compare_and_swap(&(pipe->self),0,pipe);	/** Will only succed once */
	if(__sync_fetch_and_add(&(pipe->mapped),1))
		return pipe;
	vine_list_init(&(pipe->accelerator_list));
	vine_list_init(&(pipe->process_list));
	pipe->allocator = vine_alloc_init(&(pipe->allocator)+1,size-sizeof(*pipe));
	pipe->queue = vine_alloc_alloc(pipe->allocator,queue_calc_bytes(ring_size));
	if(!pipe->queue)
		return 0;
...
{
	vine_list_add(&(pipe->accelerator_list),&(accel->list));
	return 0;
}

vine_accel_s * vine_proc_find_accel(vine_pipe_s * pipe,const char * name,vine_accel_type_e type)
{
	vine_list_node_s * itr;
	vine_accel_s * accel;
	vine_list_for_each(pipe->accelerator_list,itr)
	{
...
{
	vine_list_add(&(pipe->process_list),&(proc->list));
	return 0;
}
...
vine_proc_s * vine_proc_find_proc(vine_pipe_s * pipe,const char * name,vine_accel_type_e type)
{
	vine_list_node_s * itr;
	vine_proc_s * proc;
	vine_list_for_each(pipe->process_list,itr)
	{
