...
void *init_data_mark_done(void* data){
    ...
    vine_accel_size_inc(((vine_vaccel_s*)(((vine_data_s*)inputs[0])->accel))->phys, sizeof(boom_in)+ sizeof(boom_out));
...
START_TEST(test_single_phys_task_issue_without_wait)
{
    ...
    //initAccelerator
    accel = vine_accel_init(mypipe, "FakeAccel", accelType,  GPU_SIZE*2, GPU_SIZE*2);
    ck_assert(accel!=0);
    ...
    ck_assert_int_eq(vine_accel_get_available_size(accel), GPU_SIZE*2);
